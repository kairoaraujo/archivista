...
func (apc *AttestationPolicyCreate) AddSubjectScopeIDs(ids ...int) *AttestationPolicyCreate {
	apc.mutation.AddSubjectScopeIDs(ids...)
	return apc
}

// AddSubjectScopes adds the "subject_scopes" edges to the SubjectScope entity.
func (apc *AttestationPolicyCreate) AddSubjectScopes(s ...*SubjectScope) *AttestationPolicyCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return apc.AddSubjectScopeIDs(ids...)
}
...
func (apc *AttestationPolicyCreate) createSpec() (*AttestationPolicy, *sqlgraph.CreateSpec) {
	...
	if nodes := apc.mutation.SubjectScopesIDs(); len(nodes) > 0 {
	...
	if nodes := apc.mutation.StatementIDs(); len(nodes) > 0 {
		...
		_node.statement_policies = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}
