...
func (c *Client) init() {
	...
	c.SubjectScope = NewSubjectScopeClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		SubjectScope:          NewSubjectScopeClient(cfg),
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *SubjectScopeMutation:
		return c.SubjectScope.mutate(ctx, m)
...
// QuerySubjectScopes queries the subject_scopes edge of a AttestationPolicy.
func (c *AttestationPolicyClient) QuerySubjectScopes(ap *AttestationPolicy) *SubjectScopeQuery {
	query := (&SubjectScopeClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := ap.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(attestationpolicy.Table, attestationpolicy.FieldID, id),
			sqlgraph.To(subjectscope.Table, subjectscope.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, attestationpolicy.SubjectScopesTable, attestationpolicy.SubjectScopesColumn),
		)
...
func (c *AttestationPolicyClient) QueryStatement(ap *AttestationPolicy) *StatementQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(attestationpolicy.Table, attestationpolicy.FieldID, id),
			sqlgraph.To(statement.Table, statement.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, attestationpolicy.StatementTable, attestationpolicy.StatementColumn),
		)
		fromV = sqlgraph.Neighbors(ap.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryPolicies queries the policies edge of a Statement.
func (c *StatementClient) QueryPolicies(s *Statement) *AttestationPolicyQuery {
...
// SubjectScopeClient is a client for the SubjectScope schema.
type SubjectScopeClient struct {
	config
}

// NewSubjectScopeClient returns a client for the SubjectScope from the given config.
func NewSubjectScopeClient(c config) *SubjectScopeClient {
	return &SubjectScopeClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `subjectscope.Hooks(f(g(h())))`.
func (c *SubjectScopeClient) Use(hooks ...Hook) {
	c.hooks.SubjectScope = append(c.hooks.SubjectScope, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `subjectscope.Intercept(f(g(h())))`.
func (c *SubjectScopeClient) Intercept(interceptors ...Interceptor) {
	c.inters.SubjectScope = append(c.inters.SubjectScope, interceptors...)
}

// Create returns a builder for creating a SubjectScope entity.
func (c *SubjectScopeClient) Create() *SubjectScopeCreate {
	mutation := newSubjectScopeMutation(c.config, OpCreate)
	return &SubjectScopeCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of SubjectScope entities.
func (c *SubjectScopeClient) CreateBulk(builders ...*SubjectScopeCreate) *SubjectScopeCreateBulk {
	return &SubjectScopeCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *SubjectScopeClient) MapCreateBulk(slice any, setFunc func(*SubjectScopeCreate, int)) *SubjectScopeCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &SubjectScopeCreateBulk{err: fmt.Errorf("calling to SubjectScopeClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*SubjectScopeCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for SubjectScope.
func (c *SubjectScopeClient) Update() *SubjectScopeUpdate {
	mutation := newSubjectScopeMutation(c.config, OpUpdate)
	return &SubjectScopeUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *SubjectScopeClient) UpdateOne(ss *SubjectScope) *SubjectScopeUpdateOne {
	mutation := newSubjectScopeMutation(c.config, OpUpdateOne, withSubjectScope(ss))
	return &SubjectScopeUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *SubjectScopeClient) UpdateOneID(id int) *SubjectScopeUpdateOne {
	mutation := newSubjectScopeMutation(c.config, OpUpdateOne, withSubjectScopeID(id))
	return &SubjectScopeUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for SubjectScope.
func (c *SubjectScopeClient) Delete() *SubjectScopeDelete {
	mutation := newSubjectScopeMutation(c.config, OpDelete)
	return &SubjectScopeDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *SubjectScopeClient) DeleteOne(ss *SubjectScope) *SubjectScopeDeleteOne {
	return c.DeleteOneID(ss.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *SubjectScopeClient) DeleteOneID(id int) *SubjectScopeDeleteOne {
	builder := c.Delete().Where(subjectscope.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &SubjectScopeDeleteOne{builder}
}

// Query returns a query builder for SubjectScope.
func (c *SubjectScopeClient) Query() *SubjectScopeQuery {
	return &SubjectScopeQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeSubjectScope},
		inters: c.Interceptors(),
	}
}

// Get returns a SubjectScope entity by its id.
func (c *SubjectScopeClient) Get(ctx context.Context, id int) (*SubjectScope, error) {
	return c.Query().Where(subjectscope.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *SubjectScopeClient) GetX(ctx context.Context, id int) *SubjectScope {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryAttestationPolicy queries the attestation_policy edge of a SubjectScope.
func (c *SubjectScopeClient) QueryAttestationPolicy(ss *SubjectScope) *AttestationPolicyQuery {
	query := (&AttestationPolicyClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := ss.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(subjectscope.Table, subjectscope.FieldID, id),
			sqlgraph.To(attestationpolicy.Table, attestationpolicy.FieldID),
...
// Hooks returns the client hooks.
func (c *SubjectScopeClient) Hooks() []Hook {
	return c.hooks.SubjectScope
}

// Interceptors returns the client interceptors.
func (c *SubjectScopeClient) Interceptors() []Interceptor {
	return c.inters.SubjectScope
}

func (c *SubjectScopeClient) mutate(ctx context.Context, m *SubjectScopeMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&SubjectScopeCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&SubjectScopeUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&SubjectScopeUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&SubjectScopeDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown SubjectScope mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		...
		Signature, Statement, Subject, SubjectDigest, SubjectScope,
		Timestamp []ent.Hook
	}
	inters struct {
		Attestation, AttestationCollection, AttestationPolicy, Dsse, PayloadDigest,
