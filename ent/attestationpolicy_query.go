...
// QuerySubjectScopes chains the current query on the "subject_scopes" edge.
func (apq *AttestationPolicyQuery) QuerySubjectScopes() *SubjectScopeQuery {
	query := (&SubjectScopeClient{config: apq.config}).Query()
...
func (apq *AttestationPolicyQuery) Clone() *AttestationPolicyQuery {
	...
	return &AttestationPolicyQuery{
		...
		withSubjectScopes: apq.withSubjectScopes.Clone(),
...
// WithSubjectScopes tells the query-builder to eager-load the nodes that are connected to
// the "subject_scopes" edge. The optional arguments are used to configure the query builder of the edge.
func (apq *AttestationPolicyQuery) WithSubjectScopes(opts ...func(*SubjectScopeQuery)) *AttestationPolicyQuery {
	query := (&SubjectScopeClient{config: apq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	apq.withSubjectScopes = query
	return apq
}
...
func (apq *AttestationPolicyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AttestationPolicy, error) {
	...
	if query := apq.withSubjectScopes; query != nil {
		if err := apq.loadSubjectScopes(ctx, query, nodes,
			func(n *AttestationPolicy) { n.Edges.SubjectScopes = []*SubjectScope{} },
			func(n *AttestationPolicy, e *SubjectScope) { n.Edges.SubjectScopes = append(n.Edges.SubjectScopes, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range apq.withNamedSubjectScopes {
		if err := apq.loadSubjectScopes(ctx, query, nodes,
			func(n *AttestationPolicy) { n.appendNamedSubjectScopes(name) },
			func(n *AttestationPolicy, e *SubjectScope) { n.appendNamedSubjectScopes(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (apq *AttestationPolicyQuery) loadSubjectScopes(ctx context.Context, query *SubjectScopeQuery, nodes []*AttestationPolicy, init func(*AttestationPolicy), assign func(*AttestationPolicy, *SubjectScope)) error {
	...
	query.withFKs = true
	query.Where(predicate.SubjectScope(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attestationpolicy.SubjectScopesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.attestation_policy_subject_scopes
		if fk == nil {
			return fmt.Errorf(`foreign-key "attestation_policy_subject_scopes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "attestation_policy_subject_scopes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (apq *AttestationPolicyQuery) loadStatement(ctx context.Context, query *StatementQuery, nodes []*AttestationPolicy, init func(*AttestationPolicy), assign func(*AttestationPolicy, *Statement)) error {
	...
	nodeids := make(map[int][]*AttestationPolicy)
	for i := range nodes {
		if nodes[i].statement_policies == nil {
			continue
		}
		fk := *nodes[i].statement_policies
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "statement_policies" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
// WithNamedSubjectScopes tells the query-builder to eager-load the nodes that are connected to the "subject_scopes"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (apq *AttestationPolicyQuery) WithNamedSubjectScopes(name string, opts ...func(*SubjectScopeQuery)) *AttestationPolicyQuery {
	query := (&SubjectScopeClient{config: apq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if apq.withNamedSubjectScopes == nil {
		apq.withNamedSubjectScopes = make(map[string]*SubjectScopeQuery)
	}
	apq.withNamedSubjectScopes[name] = query
	return apq
}
