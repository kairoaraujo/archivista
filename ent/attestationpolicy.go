...
type AttestationPolicy struct {
	...
	statement_policies *int
	selectValues       sql.SelectValues
}

// AttestationPolicyEdges holds the relations/edges for other nodes in the graph.
type AttestationPolicyEdges struct {
	// SubjectScopes holds the value of the subject_scopes edge.
	SubjectScopes []*SubjectScope `json:"subject_scopes,omitempty"`
	...
	namedSubjectScopes map[string][]*SubjectScope
}

// SubjectScopesOrErr returns the SubjectScopes value or an error if the edge
// was not loaded in eager-loading.
func (e AttestationPolicyEdges) SubjectScopesOrErr() ([]*SubjectScope, error) {
	if e.loadedTypes[0] {
		return e.SubjectScopes, nil
	}
	return nil, &NotLoadedError{edge: "subject_scopes"}
}
...
func (*AttestationPolicy) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case attestationpolicy.ForeignKeys[0]: // statement_policies
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case attestationpolicy.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field statement_policies", value)
			} else if value.Valid {
				ap.statement_policies = new(int)
				*ap.statement_policies = int(value.Int64)
			}
		default:
...
// QuerySubjectScopes queries the "subject_scopes" edge of the AttestationPolicy entity.
func (ap *AttestationPolicy) QuerySubjectScopes() *SubjectScopeQuery {
	return NewAttestationPolicyClient(ap.config).QuerySubjectScopes(ap)
}
...
// NamedSubjectScopes returns the SubjectScopes named value or an error if the edge was not
// loaded in eager-loading with this name.
func (ap *AttestationPolicy) NamedSubjectScopes(name string) ([]*SubjectScope, error) {
	if ap.Edges.namedSubjectScopes == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := ap.Edges.namedSubjectScopes[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (ap *AttestationPolicy) appendNamedSubjectScopes(name string, edges ...*SubjectScope) {
	if ap.Edges.namedSubjectScopes == nil {
		ap.Edges.namedSubjectScopes = make(map[string][]*SubjectScope)
	}
	if len(edges) == 0 {
		ap.Edges.namedSubjectScopes[name] = []*SubjectScope{}
	} else {
		ap.Edges.namedSubjectScopes[name] = append(ap.Edges.namedSubjectScopes[name], edges...)
	}
}
